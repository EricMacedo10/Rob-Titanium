...
def upload_to_hostinger(local_file_path, ftp_host, ftp_user, ftp_pass, remote_path='data.json'):
    ...
    if not ftp_user or ftp_user == "seu_usuario_ftp":
        print("--- FTP nao configurado. O arquivo foi salvo apenas localmente.")
        return False
    ...
    try:
        ...
        pwd = session.pwd()
        print(f"   --- Diretorio inicial (pwd): {pwd}")
        ...
        if not entered:
            print("   --- Nenhuma das pastas padrao (public_html, domains...) foi encontrada. Ficando na raiz.")
        else:
            ...
            if env_mode == "STAGING":
                try:
                    ...
                    try:
                        ...
                        print(f"   --- Falha ao entrar na pasta de staging: {e}")

        # List files for debugging (using LIST for more detail)
        print(f"   --- Listando arquivos no diretorio...")
        files_detailed = []
        ...
        if any('data.json' in line for line in files_detailed):
            print(f"   --- data.json ja existe no servidor (sera sobrescrito)")
        else:
            print(f"   --- data.json nao encontrado (sera criado)")
        ...
        try:
            ...
            if remote_size == file_size:
                ...
                print(f"   --- ATENCAO: Tamanhos diferentes! Local: {file_size}, Remoto: {remote_size}")
        except:
            print(f"   --- Nao foi possivel verificar tamanho remoto")
        
        session.quit()
        print("--- Upload concluido com sucesso! O site foi atualizado.")
        return True
